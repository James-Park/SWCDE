...
gcm.sbm._preSubmitFunction = function(sbmObj) {
	if (com.util.isEmpty(gcm.CONTEXT_PATH) === false) {
		sbmObj.action = gcm.CONTEXT_PATH + sbmObj.action;
	}
	
	gcm.sbm._setActionParam(sbmObj)
};
 ...
 */
gcm.sbm._setActionParam = function(sbmObj) {
	...
	if (typeof requestData === "string") {
		requestData = com.util.getJSON(requestData);
	}
	...
	if (com.util.isJSON(requestData)) {
		...
		for (var i = 0; i < actionArr.length; i++) {
			if (actionArr[i].match(/(\${[^}]*})/g) !== null) {
				var paramKey = actionArr[i].substr(2, actionArr[i].indexOf("}")-2);
...
/**
 * GET 방식으로 전송할 경우 DataMap의 정보를 QueryString으로 생성해서 URL에 붙이도록 처리한다.
 * 
 ...
 * @author Inswave Systems
 */
gcm.sbm._setAction2 = function(sbmObj, requestData) {
	if (typeof requestData === "string") {
		requestData = com.util.getJSON(requestData);
	}
	...
	// requestData를 QueryString으로 생성해서 submission.action에 추가한다.
	if ((sbmObj.method === "get") && com.util.isJSON(requestData)) {
		for ( var key in requestData) {
			if ((com.util.isEmpty(requestData[key]) === false) && (com.util.isJSON(requestData[key]) === false)) {
				if (com.util.isEmpty(paramUrl) === false) {
					paramUrl += "&";
				}

				paramUrl += key + "=" + encodeURIComponent(requestData[key]);
			} else if ((com.util.isJSON(requestData[key])) && (com.util.isArray(requestData[key]) === false)) {
				for ( var subKey in requestData[key]) {
					if ((com.util.isEmpty(requestData[key][subKey]) === false) && (com.util.isJSON(requestData[key][subKey]) === false)) {
						if (com.util.isEmpty(paramUrl) === false) {
						...
						paramUrl += subKey + "=" + encodeURIComponent(requestData[key][subKey]);
					}
				}
			}
		}
		...
		if (sbmObj.__action.indexOf("?") > 0) {
			sbmObj.action = sbmObj.__action + "&" + paramUrl;
		} else {
			sbmObj.action = sbmObj.__action + "?" + paramUrl;
		}
	}
